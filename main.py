...
# ================= 战争事件 =================
WAR_EVENTS = [
    {"title":"战前动员","text":"上级下达战备命令，你部进入一级战备。",
     "choices":[
    ...
    {"title":"相持阶段","text":"连续作战多日，部队疲惫，弹药给养告急。",
     "choices":[
    ...
    {"title":"敌军反扑","text":"敌军集中优势兵力反扑，你部承受巨大压力。",
     "choices":[
     ...
     "decisions":[
        ...
        {"title":"决策6/6 · 俘虏审讯","text":"抓到一名敌军军官。",
         "choices":[
            ...
            {"text":"无视继续战斗","score":-1,"effects":{},"note":"错失情报"}]},
     ]},
...
SUNSET_NATIONS = {
    "我国":{"role":"我方","desc":"祖国"},
    "敌国A":{"role":"主要敌国","desc":"岛国"},
    "域外大国B":{"role":"域外干预大国","desc":"远洋强国"},
...
class MusicManager:
    ...
    def play(self, name):
        ...
        path = None
        for ext in (".ogg", ".wav"):
            test = os.path.join(MUSIC_DIR, base + ext)
            if os.path.exists(test):
                path = test
                break
        print(f"[音乐] 请求 {name} -> 路径 {path}")
        if not path:
            print(f"[音乐] 文件不存在")
            self.current = None
            self.current_name = None
...
class GameScreen(FloatLayout):
    ...
    def update_status(self):
        ...
        self.set_portrait(get_rank_image(s["rank_index"], s.get("at_war", False)))
    ...
    def step1_register(self, education, min_age, max_age):
        name = "张伟"
        if self.name_input:
            name = self.name_input.text.strip() or "张伟"
    ...
    def step3_initial_check(self):
        ...
        if min(left_eye, right_eye) < 4.5: problems.append("裸眼视力低于4.5")
        text = (
            f"【征兵流程 · 第3步：初审初检】\n\n"
            f"━━━━━━━━━━━━━━━━━\n"
            f"  身高：{height} cm\n  体重：{base_weight} kg\n"
    ...
    def step4_medical(self):
        ...
        if random.random() < 0.2:
            hidden = random.choice(["心电图轻微异常", "血压偏高", "肝功指标偏高", "扁平足"])
        text = (
            f"【征兵流程 · 第4步：体格检查】\n\n"
            f"━━━━━━━━━━━━━━━━━\n"
            f"  身高：{final_height} cm\n  BMI：{final_bmi:.1f}\n  视力：{final_eye:.1f}\n"
            f"━━━━━━━━━━━━━━━━━\n\n")
        if problems or hidden:
            text += "【体检结论】❌ 不合格\n\n"
            for p2 in problems:
                text += f"  ✗ {p2}\n"
            if hidden:
    ...
    def step8_enlist(self):
        ...
        for name, attrs in PROVINCES:
            ...
            self.add_choice(f"{name}  [{attr_str}]", lambda n=name, a=attrs: self.pick_province(n, a))
    ...
    def do_task(self, task):
        ...
        if self.state["health"] < 50 and not self.state.get("in_hospital"):
            Clock.schedule_once(lambda dt: self.go_to_hospital(), 0.3)
            return
        if random.random() < 0.4:
    ...
    def resolve_event(self, ev, choice):
        ...
        self.update_status()
        if self.state["health"] < 50 and not self.state.get("in_hospital"):
            Clock.schedule_once(lambda dt: self.go_to_hospital(), 0.5)
            return
        Clock.schedule_once(lambda dt: self.advance_time(), 0.5)
    ...
    def war_next_step(self):
        ...
        self.text.text = (
            f"【战争 · {ev['title']}】\n\n{ev['text']}\n\n"
            f"━━━━━━━━━━━━━━━━━\n第 {self.war_round+1}/{len(WAR_EVENTS)} 阶段\n"
    ...
    def transfer_hospital(self):
        ...
        if idx >= len(keys) - 1:
            ...
            self.clear_choices()
            self.add_choice("返回", self.show_hospital)
            return
    ...
    def show_democratic_review(self):
        ...
        if score >= 85:
            level, desc, rep_gain, rel_gain = "优秀", "战士们一致认为你德才兼备。", 8, 5
        elif score >= 65:
            level, desc, rep_gain, rel_gain = "良好", "战士们对你评价不错。", 4, 2
        elif score >= 40:
            level, desc, rep_gain, rel_gain = "合格", "评价中规中矩。", 0, 0
        else:
            level, desc, rep_gain, rel_gain = "不合格", "战士们对你意见很大。", -8, -8
    ...
    def check_achievements(self):
        ...
        for key, ach in ACHIEVEMENTS.items():
            ...
            try:
                if ach["check"](s):
                    unlocked[key] = {"name": ach["name"], "desc": ach["desc"], "year": s["years"]}
                    if "reward" in ach:
        ...
        if not lover:
            ...
            if s["age"] >= 20:
                ...
                for m in MEET_METHODS:
                    self.add_choice(f"{m} — {MEET_METHODS[m]['desc']}", lambda x=m: self.try_meet(x))
                self.add_choice("返回", self.show_actions)
            else:
                ...
                self.add_choice("返回", self.show_actions)
            return
        self.text.text += (f"恋人：{lover['name']}\n职业：{lover['work']}\n性格：{lover['traits']}\n"
                           f"状态：{self.love_mood()}\n好感度：{s.get('lover_affection',0)}\n\n")
        if s.get("married"):
            self.text.text += "【已婚】\n"
    ...
    def give_gift(self, name, money, aff, hc=0):
        ...
        self.music.play("family.wav")
        lover = s.get("lover")
        self.text.text = f"【送礼物 · {name}】\n\n你把{name}送给{lover['name']}。\n好感+{aff}"
        self.update_status()
        self.clear_choices()
        self.add_choice("返回", self.show_love_menu)
    ...
    def trigger_love_event(self):
        ...
        if random.random() < 0.4:
            ...
            if event == "结婚纪念日" and s.get("married"):
                ...
                self.clear_choices()
                self.add_choice("请假回家庆祝", self.visit_family)
                self.add_choice("寄礼物回去", lambda: self.give_gift("纪念礼物", 500, 8, 0))
                return True
            if event == "孩子出生" and s.get("married") and s.get("children", 0) == 0:
                s["children"] = s.get("children", 0) + 1
                self.music.play("family.wav")
                self.text.text = f"【孩子出生】\n\n{lover['name']}给你打来电话，\n孩子的哭声透过听筒传来。\n\n你当爸爸了！"
                self.update_status()
                self.clear_choices()
                self.add_choice("继续", self.show_actions)
                return True
